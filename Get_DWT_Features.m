function [ feature_matrix, num, coeff ] = Get_DWT_Features( subj_name, is_train )
...
cd(['/Volumes/My Passport/EEG_Project/' subj_name]);
...
if is_train
    ...
    for i = 3:num_interictal+2
        ...
        dwt_features = zeros(num_channels,num_features);
        for ch = 1:num_channels
            ...
            for d = 1:decomp_level+1
                ...
                dwt_features(ch,feature_start+1) = mean(abs(C(indeces)));
                dwt_features(ch,feature_start+2) = mean(C(indeces).^2);
                dwt_features(ch,feature_start+3) = std(C(indeces));
            end
            save([subj_name '_interictal_coefficients_' num2str(i-2)],'coefficients');
            ...
            for d = 1:decomp_level
                ...
                dwt_features(ch,ratio_start+d) = dwt_features(ch,index_1) / dwt_features(ch,index_2);
            end
        end
        feature_matrix(i-2,:,:) = dwt_features;
    end
    ...
    for i = 3:num_preictal+2
        ...
        for ch = 1:num_channels
            ...
            save([subj_name '_preictal_coefficients_' num2str(i-2)],'coefficients');
            ratio_start = (features_per_decomp-1)*(decomp_level+1);
            for d = 1:decomp_level
                index_1 = (features_per_decomp-1)*(d-1)+1;
                index_2 = (features_per_decomp-1)*d+1;
        ...
        feature_matrix(num_interictal+i-2,:,:) = dwt_features;
    end
    num = num_interictal;
else
    cd('test');
...
cd('~/Dropbox/EEG_Project/cs221-cs229-eeg');